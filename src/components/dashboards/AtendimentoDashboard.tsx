const AtendimentoDashboard = () => {
  const statusData = [
    { label: "Resolvido", pct: 62, color: "from-emerald-500/40 to-emerald-400/60" },
    { label: "Em andamento", pct: 24, color: "from-cyan-500/40 to-cyan-400/60" },
    { label: "Pendente", pct: 14, color: "from-amber-500/40 to-amber-400/50" },
  ];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { label: "Abertos", value: "128", color: "text-foreground" },
          { label: "Resolvidos", value: "112", color: "text-emerald-400" },
          { label: "SLA Atingido", value: "94%", color: "text-cyan-400" },
          { label: "Tempo Médio", value: "2.4h", color: "text-foreground" },
        ].map((kpi, i) => (
          <div key={i} className="dash-kpi">
            <div className="text-[7px] font-mono text-muted-foreground leading-none mb-1">{kpi.label}</div>
            <div className={`text-[13px] font-bold font-mono leading-none ${kpi.color}`}>{kpi.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="dash-inner p-3">
        ...
        <div className="flex items-end gap-[3px] h-14">
          {[8, 12, 6, 15, 10, 14, 9, 11, 7, 13, 16, 8, 12, 10, 15, 9, 11, 14, 7, 13].map((v, i) => (
            <div
              key={i}
              className="flex-1 rounded-t-sm bg-gradient-to-t from-cyan-600/25 to-cyan-400/40 transition-all"
      ...
      <div className="dash-inner overflow-hidden">
        ...
        {[
          ...
          <div key={i} className="grid grid-cols-4 text-[7px] font-mono text-foreground/80 px-3 py-1.5 border-b border-white/[0.02]">
            {row.map((c, j) => (
              <span key={j} className={j === 3 && parseFloat(c) >= 95 ? "text-emerald-400" : ""}>{c}</span>
            ))}
          </div>
        ))}
      </div>
